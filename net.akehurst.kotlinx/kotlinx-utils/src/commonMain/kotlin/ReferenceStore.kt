...
class HierarchicalReferenceStoreByHashMap<R : Any>(
    ...
    override fun <T : Any> getQualifiedReference(clazz: KClass<T>, qualifiedReference: List<R>): T? = when {
        1 == qualifiedReference.size -> error("A Reference needs the identity of its factory!")
        identity != qualifiedReference[0] -> error("Reference does not match the identity of factory!")
